...
Get.Matched.ID <- function(queryID,metadata,ByColname,keepCol=NULL) {
  ...
  for(i in 1:length(queryID)){
    ...
    for(j in 1:dim(metadata)[1]){
      if (!is.na(Match.ID(metadata[[ByColname]][j],queryID[i])[[1]])){
        Matched <- Match.ID(metadata[[ByColname]][j], queryID[i])
        Matched.ID[i] <- Matched[[1]]
        break
      } 
    }
  ...
  if (!is.null(keepCol)) {
    ...
    cat("Returning matched IDs along with specified metadata columns.\n\n")
    Matched.Data
  } else {
    Matched.Data <- metadata[match(Matched.ID,metadata[[ByColname]]),]
    cat("Returning all metadata columns with match ID. No additional columns specified to keep.\n\n")
    Matched.Data
  }
  # print the unmatched IDs
  Unmatched.ID <- metadata[[ByColname]][!metadata[[ByColname]] %in% Matched.ID]
